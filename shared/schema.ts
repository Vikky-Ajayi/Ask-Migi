...
export const userProfiles = pgTable("user_profiles", {
  ...
  experiences: jsonb("experiences").$type<Array<Record<string, unknown>>>().notNull().default(sql`'[]'::jsonb`),
  education: jsonb("education").$type<Array<Record<string, unknown>>>().notNull().default(sql`'[]'::jsonb`),
  certifications: jsonb("certifications").$type<Array<Record<string, unknown>>>().notNull().default(sql`'[]'::jsonb`),
  references: jsonb("references").$type<Array<Record<string, unknown>>>().notNull().default(sql`'[]'::jsonb`),
  profileStep: integer("profile_step").notNull().default(0),
