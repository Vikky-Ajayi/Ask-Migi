...
export const BuyCoinsPage = (): JSX.Element => {
  ...
  const purchaseMutation = useMutation({
    ...
    onSuccess: async (data, variables) => {
      await refreshUser();
      toast({ title: "Coins added!", description: `${variables.coinsAmount} coins have been added to your account.` });
    },
    onError: (e: any) => {
    ...
    <main className="min-h-screen w-full bg-[#161618] text-white flex flex-col">
      ...
      <div className="flex flex-col items-center px-4 md:px-6 py-10 md:py-12">
        <div className="text-center mb-8 md:mb-10">
          <h1 className="text-3xl md:text-5xl font-bold text-white tracking-tight">Buy coins</h1>
          <p className="mt-3 text-sm md:text-base text-white/50">Coins helps you get answers to your questions</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-5 w-full max-w-4xl">
          {coinPacks.map((pack) => (
            <div
              key={pack.coins}
              className="rounded-3xl border border-[#3a3c3e] bg-[#1e2022] p-6 md:p-7 flex flex-col gap-4 md:gap-5"
            ...
            >
              ...
              <p className="text-4xl md:text-5xl font-bold text-white leading-none text-center">{pack.price}</p>
