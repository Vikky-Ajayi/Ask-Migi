...
export const ChatPage = (): JSX.Element => {
  ...
  const messages = [
    { id: 1, type: "user", text: "Hello, Can I get help regarding immigration to the UK" },
  ];
      ...
      <div className="mx-auto flex min-h-screen w-full max-w-[375px] flex-col px-4 pb-4">
        ...
        <div className="flex flex-col gap-4 mt-6 flex-1">
          {messages.map((msg) => (
            <div key={msg.id} className="flex justify-end">
              <div className="max-w-[80%] rounded-2xl rounded-tr-sm bg-[#2a2c2e] border border-[#3a3c3e] px-4 py-3">
                <p className="text-sm text-white/90 leading-5">{msg.text}</p>
              </div>
            </div>
          ))}

          {/* Expert pending response */}
          <div className="flex flex-col gap-2">
            ...
            <p className="text-sm text-white/70 leading-5 ml-10">
              ...
              {audienceOptions.map((option) => {
                ...
                return (
                  <button
                    ...
                    data-testid={`chat-tab-${option.toLowerCase().replace(/\s+/g, "-")}`}
                  >
                    {option}
                  </button>
                );
