  ...
  return (
    <span>
      ...
      {displayed.length < text.length && (
        <span className="inline-block w-[2px] h-[1em] bg-th-text-50 align-middle ml-0.5 animate-pulse" />
      )}
    </span>
  );
}
            ...
            <div className="mx-auto w-full max-w-2xl flex flex-col gap-6">
                    ...
                    {isAnswered && activeEnquiry.answer ? (
                      <div className="ml-10">
                        {activeEnquiry.answer.split("\n\n").map((para: string, i: number) => (
                          <p key={i} className="text-sm text-th-text-80 leading-6 mb-3 last:mb-0">
                            {para}
                          </p>
                        ))}
                        {activeEnquiry.answeredBy && (
                          <p className="text-xs text-th-text-30 mt-3 italic">
                            — {activeEnquiry.answeredBy}
                          </p>
                        )}
                        <div className="mt-5 flex items-center gap-2.5 px-4 py-3 rounded-xl bg-th-hover border border-th-border-md">
                          <img src={coinImg} alt="coins" className="w-5 h-5 object-contain shrink-0" />
                          <p className="text-xs text-th-text-60 leading-5">
                ...
                <div key={i} className="flex flex-col gap-4">
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-semibold text-th-text-70">Ask MiGi</span>
                    </div>
                    <div className="ml-10">
              ...
              {casualMutation.isPending && (
                <div className="flex flex-col gap-4">
                  <div className="flex justify-end">
                    <div className="max-w-[85%] rounded-2xl rounded-tr-sm bg-th-card-hover border border-th-border-md px-4 py-3">
                      <p className="text-sm text-th-text-90 leading-6">{casualMutation.variables}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 ml-10">
                    <span className="text-sm text-th-text-40 animate-pulse">Ask MiGi is typing…</span>
                  </div>
                </div>
              )}
